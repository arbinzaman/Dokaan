...
const RegistrationStepper = () => {
  ...
  const [shopData, setShopData] = useState({ dokaan_name: "", dokaan_location: "", dokaan_email: "", dokaan_phone: "", dokaan_type: "" });
  const [errors, setErrors] = useState({});
  const { setUser, setDokaan } = useUser();
  const navigate = useNavigate();

  const validateData = () => {
    const validationErrors = activeStep === 0
      ? {
      ...
      : {
          ...
          dokaan_location: shopData.dokaan_location ? "" : "Shop Location is required",
  ...
  const handleSubmit = async () => {
    ...
    toast.promise(
      axios.post(`${import.meta.env.VITE_BASE_URL}/dokaan`, registrationData)
        .then(({ data }) => {
          Cookies.set("XTOKEN", data.access_token, { expires: 1, path: "/" });
          setUser(data.data.owner);
          setDokaan(data.data.dokaan);
          navigate("/dashboard");
          return data;
        }),
      { loading: "Saving...", success: "Registration successful!", error: "Registration failed!" }
    );
  };

  return (
    <Box sx={{ width: "100%", minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center", p: 3 }}>
      <Box sx={{ width: { xs: "100%", sm: "75%", md: "50%" }, backgroundColor: "gray.700", borderRadius: 2, boxShadow: 3, p: 3 }}>
        <Typography variant="h4" align="center" gutterBottom>Registration</Typography>
        <Stepper activeStep={activeStep} alternativeLabel>
          {steps.map((label, index) => (
            <Step key={index}>
              <StepLabel StepIconComponent={() => <StepIcon icon={icons[index + 1]} active={activeStep === index} completed={activeStep > index} />}>{label}</StepLabel>
            </Step>
          ))}
        </Stepper>
        <Box sx={{ mt: 3 }}>
          {activeStep === 0 ? <UserRegistrationForm userData={userData} setUserData={setUserData} errors={errors} /> : <ShopInfoForm shopData={shopData} setShopData={setShopData} errors={errors} />}
          <Box sx={{ display: "flex", justifyContent: "space-between", mt: 2 }}>
            <Button disabled={activeStep === 0} onClick={() => setActiveStep((prev) => prev - 1)}>Back</Button>
            {activeStep === steps.length - 1 ? <Button variant="contained" onClick={handleSubmit}>Submit</Button> : <Button variant="contained" onClick={handleNext}>Next</Button>}
          </Box>
        </Box>
      </Box>
    </Box>
